...
class MobileDetailPage extends React.Component{
    ...
    handleScroll=()=>{
        this.isBlur=document.getElementById(`mobileDetail${this.currentPage}`).getBoundingClientRect().top-window.screen.height
            ...
            <LargeFrame>
                ...
                <MoveFrame id="detailPageMoveFrame">
                    {this.props.data.map((item,key)=>{
                        ...
                        if(item.type==="text"){
                            return(
                                <Frame  color={"#e0e0e0"} key={'frame'+key} id={`mobileDetail${key}`} >
                                    <MobileTitleAndText data={item} key={key} blur={this.state.blurControl[key]}/>
                                </Frame>
                            )
                        }
                        if(item.type==="photo"){
                            return (
                                <Frame color={"white"} key={'frame'+key} id={`mobileDetail${key}`}>
                                    <MobilePhoto data={item} key={key}  blur={this.state.blurControl[key]}/>
                                </Frame>
                            )
                        }
                    })}
